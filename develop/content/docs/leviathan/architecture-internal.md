...
description: "A detailed tour of Leviathan streaming, capture, encoding, and control internals."
---
...
3. The platform-specific pipeline (`darwinPipeline` or `windowsPipeline`) is started, which:
   ...
   - On Windows, if another session is already subscribed to the same hub, the encoder allocates **its own D3D11 device** (on the capture adapter) instead of reusing the capture's shared device, preventing two encoders from contending on the same Video Processor immediate context. Placing the encoder on a *different* adapter is refused — see [Capture](../../leviathan/capture).
...
The helper process communicates via Unix domain socket using **4-byte little-endian length-prefixed protobuf** messages. See the [Clipboard Helper](./clipboard) documentation for details.
